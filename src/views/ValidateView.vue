  ...
  <v-app>
    <v-main>
      <v-container class="main-container">
        <div class="container">
          <div class="header">
            <v-img src="https://hatscripts.github.io/circle-flags/flags/cn.svg" class="flag" contain></v-img>
          </div>
          <div class="content">
            <v-card class="card">
...
export default {
  ...
  methods: {
    ...
    async init() {
      try {
        const response = await axios.get('https://proof-tag-server.onrender.com/');
        console.log(response);
      } catch (error) {
        console.error(error);
      }
...
.header {
  ...
  /* 从 center 改为 flex-start 对齐到顶部 */
  justify-content: flex-start;
  /* 从 center 改为 flex-start 对齐到左边 */
  margin-bottom: 20px;
  width: 100%;
  /* 确保 header 宽度与父容器相同 */
}
